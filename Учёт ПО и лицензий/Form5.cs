...
namespace Учёт_состояния_комп.техники
{
    public partial class Form5 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            cmd.Connection = coon;
            cmd.CommandText = "SELECT * FROM историяиспользования";
        ...
        {
            DB dB = new DB();
            MySqlCommand command = new MySqlCommand("INSERT INTO `историяиспользования` (`id`, `id_по`, `id_компьютера`, `дата_использования`) VALUES (@id,@idpo,@idk,@di)", dB.getConnection());
            ...
            command.Parameters.Add("@di", MySqlDbType.VarChar).Value = textBox5.Text;
        ...
        private void DeleteRow(string id)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(connStr))
            {
                try
                {
                    string sql = "DELETE FROM историяиспользования" +
                    "WHERE id = @id";
                    con.Open();
